...
import type { PaginationResult } from "@/lib/transactions/types";
import { formatNumber } from "@/lib/transactions/utils";
    ...
    <div className="flex flex-col items-center justify-between gap-3 sm:flex-row">
      ...
      <div className="flex items-center gap-2">
        <div className="flex items-center gap-1">
          ...
          {pageList.map((p, idx) =>
            p === "..." ? (
              <span key={`dots-${idx}`} className="px-1.5 text-xs text-neutral-400">…</span>
            ) : (
              <button
                key={p}
                type="button"
        ...
        <select
          ...
          className="h-8 cursor-pointer appearance-none rounded-lg border border-black/5 bg-white px-2 text-xs text-neutral-700 dark:border-white/10 dark:bg-neutral-900 dark:text-neutral-200"
        >
